...
import {
    frontMenus,
    afterMenus
} from "@/views/userInfo/menus"
export function getAuthPower(powerList, type) {
    if (type == "front") {

        let temp = []
        frontMenus.forEach(menu => {
            if (powerList.includes(menu.key)) {
                temp.push(menu)
            }
        });
        temp.push({
...
export function isbtnPower(key) {
    const userInfo = JSON.parse(localStorage.getItem("userInfo"));
    if (userInfo) {
        return userInfo.auth_menu.includes(key)
    }
    return false
}
